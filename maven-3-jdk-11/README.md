# Khulnasoft Maven (3-jdk-11)  Action

A [GitHub Action](https://github.com/features/actions) for using [Khulnasoft](https://khulnasoft.co/KhulnasoftGH) to check for
vulnerabilities in your Maven-3-jdk-11 projects. This Action is based on the [Khulnasoft CLI][cli-gh] and you can use [all of its options and capabilities][cli-ref] with the `args`.
  ...
  security:
    ...
    steps:
      ...
      - name: Run Khulnasoft to check for vulnerabilities
        uses: khulnasoft/actions/maven-3-jdk-11@master
        env:
